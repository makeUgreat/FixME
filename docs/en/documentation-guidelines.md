...
- English source documents include `last_reviewed` only when there is an actual review process.
...
- Use frontmatter to show the document language, audience, scope, and source/translation relationship.
- If `read_when` is present, keep it concrete and task-based.
...
- Reading situations differ, such as testing rules and API design rules.
- Change frequency differs, such as stable architecture principles and frequently changing API contracts.
- Applicability differs, such as frontend and backend rules.
- Audience, owner, or review flow differs.
- A long document makes agents load unrelated sections repeatedly.
- Splitting reduces the context a reader must load for real tasks.

### Keep Content Together When

- Rules and exceptions must always be read together.
- The content is too small to create meaningful routing value.
- The routing trigger would be vague, such as `best-practices.md`, `misc.md`, or `notes.md`.
- Splitting would duplicate explanations across files.

### New Document Checklist

- Can this document be routed from `index.md` with a clear task condition?
- Is this content not always read together with an existing document?
- Does splitting this document reduce the context that must be read for real tasks?
...
- Keep heading hierarchy shallow unless deeper nesting prevents repeated or unclear rules.
- Put scope and source-of-truth rules before task-specific rules.
- Put default rules before exceptions.
- Put examples, checklists, and maintenance notes after the rules they support.
- Group related rules under explicit headings instead of using one long mixed list.
- Prefer short paragraphs that introduce a section, followed by scannable rules.
