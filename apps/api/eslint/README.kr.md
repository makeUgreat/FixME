...
| Local `style` plugin                         | 모든 lint 대상 file            | Project-specific TypeScript style suggestion을 보고한다.             |
...
| Rule                            | Level | Scope               | Check                                                                                                                 |
| ------------------------------- | ----- | ------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `style/prefer-inline-satisfies` | warn  | 모든 lint 대상 file | Typed `const` object literal이 바로 다음 `return` call의 직접 인자로만 쓰이면 `satisfies`로 inline하는 것을 권장한다. |
...
| `mapper/prefer-application-error-of`   | error | `src/**/*.ts` | Application error contract는 shape를 직접 풀어 쓰지 않고 `ApplicationErrorOf<Kind, Owner, Reason, Details>`를 사용해야 한다.    |
