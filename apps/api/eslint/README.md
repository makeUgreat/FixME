...
| `domain/domain-error-shape` | error | `src/**/*.ts` except shared `error.type.ts` | Domain `err({ ... })` objects must include `kind`, `code`, `message`, and `details`, and use supported domain error categories. |
| `domain/no-global-domain-error-codes` | error | `src/libs/ddd/error.type.ts` | Shared `DomainError` must not become a registry of feature-domain error codes. |
| `domain/split-multiple-validation-errors` | error | `src/**/*.ts` | `validateProps` should split multiple direct validation failures into named validation methods. |
| `domain/require-unit-spec` | error | `src/**/*.aggregate.ts`, `src/**/*.entity.ts`, `src/**/*.vo.ts` | Domain model files must have colocated unit specs. |
...
| Rule | Level | Scope | Check |
| --- | --- | --- | --- |
| `mapper/implements-layer-mapper` | error | `src/**/*.ts` | Infrastructure, application error, and presentation mapper classes must implement their layer-specific mapper interface. |
