...
| `domain/no-global-domain-error-codes`     | error | `src/libs/ddd/error.base.ts`                                    | Shared `DomainError` must not become a registry of feature-domain error codes.                                                                             |
...
| `test/integration-adapter-target-file-name` | error | `src/**/*.spec.ts`, `test/**/*.ts` | Integration spec file names must identify the adapter target.                                           |
