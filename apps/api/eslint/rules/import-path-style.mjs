...
function getExpectedAlias(sourcePath, targetPath) {
  ...
  const layerKernelMatch = targetPath.match(/^src\/layer-kernels\/([^/]+)(?:\/.*)?$/);

  if (layerKernelMatch) {
    const sourceKernelPrefix = `src/layer-kernels/${layerKernelMatch[1]}`;

    if (startsWithPath(sourcePath, sourceKernelPrefix)) {
      return null;
    }

    return targetPath.replace(/^src\/layer-kernels/, '@layer-kernels');
  }

  const domainMatch = targetPath.match(/^src\/contexts\/([^/]+)\/domain(?:\/.*)?$/);
  ...
  if (startsWithPath(targetPath, 'src/bootstrap')) {
    if (startsWithPath(sourcePath, 'src/bootstrap')) {
      return null;
    }

    return targetPath.replace(/^src\/bootstrap/, '@bootstrap');
  }
