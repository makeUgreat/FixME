...
const requireUnitSpecRule = {
  meta: {
    ...
    docs: {
      description:
        'Require concrete domain model files to have a colocated __tests__ unit spec.',
    },
    messages: {
      missingUnitSpec:
        'Domain models must have a colocated __tests__ unit spec file: {{specPath}}.',
    },
    schema: [],
  },
  create(context) {
    ...
    return {
      Program(node) {
        context.report({
          ...
          data: {
            specPath: path.join('__tests__', path.basename(specPath)),
          },
        });
      },
    };
