...
- Use `toPrimitives` for serialization to persistence/API boundaries. Use `from` or `fromX` for conversion from another in-memory representation.
...
- Use `PascalCase` for types, interfaces, classes, and enums. Do not use `I` prefixes for interfaces.
